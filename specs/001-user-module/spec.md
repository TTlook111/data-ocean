...
- **FR-010**: 系统 MUST 支持管理员重置用户密码（生成临时密码）
- **FR-011**: 系统 MUST 支持已登录用户修改自身密码（需验证旧密码）
- **FR-012**: 系统 MUST 强制密码复杂度规则（最少 8 位，必须包含字母和数字）
- **FR-013**: 系统 MUST 在用户首次登录时强制修改密码
- **FR-014**: 系统 MUST 允许用户查看和编辑自身个人资料（realName/email/phone）
