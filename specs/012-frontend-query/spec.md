...
业务用户登录后默认进入 AI 问答工作台，先选择一个已授权数据源，再在该数据源作用域下输入自然语言问题并等待系统返回结果。
...
1. **Given** 用户登录成功, **When** 跳转完成, **Then** 默认进入 `/query` AI 问答工作台
2. **Given** 用户打开问答页面, **When** 页面加载完成, **Then** 左侧显示数据源选择器（仅展示有权限的已启用数据源）和当前数据源下的历史会话列表，中间显示对话区和输入框
3. **Given** 用户选择数据源并输入问题, **When** 点击发送, **Then** 显示查询进度（SSE 实时推送）
4. **Given** 查询完成, **When** 结果返回, **Then** 展示数据表格和推荐图表
...
用户在同一数据源的同一会话中连续提问，系统理解上下文关联；不同数据源拥有各自独立的历史会话。
...
2. **Given** 用户切换数据源, **When** 切换完成, **Then** 左侧历史列表切换为目标数据源的会话列表，并清空当前输入态
3. **Given** 数据源 A 和数据源 B 都有历史会话, **When** 用户在 A 与 B 之间切换, **Then** 只能看到当前数据源对应的历史会话，不能混合展示
...
- **FR-001**: 前端 MUST 在登录后默认进入 `/query` AI 问答工作台
- **FR-002**: 前端 MUST 提供数据源选择器（仅展示有权限的已启用数据源），并把数据源作为会话作用域边界
- **FR-003**: 前端 MUST 提供自然语言输入框
- **FR-004**: 前端 MUST 通过 SSE 实时展示查询进度
- **FR-005**: 前端 MUST 展示数据表格和 ECharts 图表
- **FR-006**: 前端 MUST 支持图表类型切换（柱状图、折线图、饼图）
- **FR-007**: 前端 MUST 支持结果导出（表格 CSV、图表 PNG）
- **FR-008**: 前端 MUST 展示口径说明和溯源面板（使用了哪些表、字段、可信度）
- **FR-009**: 前端 MUST 支持用户反馈（赞/踩 + 原因选择）
- **FR-010**: 前端 MUST 支持多轮对话展示和历史会话查看
- **FR-011**: 前端 MUST 在 ECharts 配置无效时降级为纯表格
- **FR-012**: 系统 MUST 自动为新会话生成标题（基于第一个问题的前 20 字）
- **FR-013**: 系统 MUST 支持当前数据源下的会话搜索（按标题/问题内容模糊匹配）
- **FR-014**: 前端问答端和后台治理端为同一 Vue 项目，通过路由区分（/query/* 和 /admin/*）
- **FR-015**: 前端 MUST 在左侧历史栏按当前数据源展示会话，切换数据源时不得携带上一数据源的对话上下文

### Key Entities

- **Conversation**: 会话，包含会话ID、用户ID、数据源ID、标题、创建时间、过期时间；历史列表默认按当前数据源过滤
...
- 会话超过 24 小时无交互自动过期
- 前端维护当前数据源下当前会话最近 5 轮对话上下文，每次请求透传
