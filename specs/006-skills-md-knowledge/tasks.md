...
- [X] T028 实现定时任务（@Scheduled 每 5 分钟）扫描 status=PENDING/CLEANUP_PENDING 的 vector_index_task；初次任务调用 007 模块的 /internal/rag/vectorize，清理重试任务只调用旧向量删除接口，更新任务状态为 PROCESSING/COMPLETED/CLEANUP_PENDING/FAILED
