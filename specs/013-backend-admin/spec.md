...
1. **Given** 管理员在问答端右上角打开用户菜单, **When** 点击"后台管理", **Then** 进入治理工作台
2. **Given** 管理员登录后台, **When** 进入治理工作台, **Then** 看到数据源列表、元数据状态、待处理问题数、待审核项数
3. **Given** 管理员点击某个数据源, **When** 进入详情, **Then** 看到该数据源的元数据快照、skills.md 版本、治理状态概览
...
- 普通用户是否能看到后台入口？（无后台功能权限时右上角用户菜单不展示后台管理入口，直接访问 /admin/* 时由路由守卫和后端接口拦截）
...
- **FR-013**: 后台 MUST 提供系统设置页面（在线修改配置参数）
- **FR-014**: 后台 MUST 提供通知中心（治理告警、审核提醒）
- **FR-015**: 后台 MUST 记录管理端操作日志（谁在什么时间做了什么管理操作）
- **FR-016**: 后台入口 MUST 放在右上角用户菜单中，并仅对具备后台功能权限的用户展示
- **FR-017**: 后台菜单和操作按钮 MUST 根据当前用户 `permissions` 裁剪，后端接口仍必须做权限校验
