...
public interface FieldConfidenceService {
    ...
    /**
     * 批量查询字段可信度。显式传入的 ID 必须全部存在且在负责源内，任一无权/不存在即整批拒绝。
     *
     * @param columnMetaIds         字段元数据ID列表，最多 200 个
     * @param visibleDatasourceIds  调用者负责源（切面准入后由 Controller 传入；Service 仍按字段真实归属逐项校验）
     * @return 可信度视图对象列表
     */
