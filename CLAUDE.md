...
mock/                  — Mock 数据目录（已提交 git，不可修改/重新生成，启动时自动加载）
...
| `knowledge/` | 006 | ✅ 完成 | 从元数据模板生成 skills.md、生成触发、生命周期管理（PENDING_REVIEW → REVIEW → APPROVED/PUBLISHED）、审核工作流（通过/驳回）、发布（版本管理/内容冻结）、知识库条目、状态筛选。向量索引定时任务。分块器（ChunkSplitter）。 |
...
- 012 问答端：刷新后历史恢复、SSE 实时进度条、取消按钮 UI、完整历史搜索。
- 003 元数据采集：真实数据源采集流程端到端验证。
- 007 Schema RAG：Milvus 真实数据源的完整集成验证（当前有 fallback 降级兜底）。
- 016 图表生成：端到端图表渲染流程验证（Python 生成 + 前端 ECharts 渲染联调）。
- 017 错误处理与降级：异常场景覆盖和降级策略验证。
...
| Java → Python | POST /internal/query/rewrite | Query Rewrite（时间解析/指代消解/意图提取） |
...
| Java → Python | POST /internal/rag/skills/generate | 触发 skills.md 生成 |
| Java → Python | GET /internal/rag/skills/{skillsId}/status | 轮询 skills.md 生成状态 |
| Java → Python | GET /internal/rag/skills/{skillsId}/content | 获取生成的 skills.md 内容 |
| Java → Python | POST /internal/rag/skills/{skillsId}/vectorize | 向量化已发布的 skills.md |
...
| **知识库** |||
| GET | /api/admin/knowledge/skills | skills.md 列表 |
| POST | /api/admin/knowledge/skills/generate | 触发 skills.md 生成 |
| **Prompt 管理** |||
| GET | /api/admin/prompts | Prompt 列表 |
| POST | /api/admin/prompts | 创建 Prompt |
| PUT | /api/admin/prompts/{id} | 编辑 Prompt |
...
- `mock/` 目录已提交 git，包含 schema-comments.json 等 Mock 数据文件
- 应用启动时自动加载 Mock 数据，无需额外配置
- **不可修改或重新生成** Mock 数据文件，确保测试环境一致性
- Mock 数据用于前端开发和联调测试，不涉及真实数据库连接

## Database Migrations

> 共 29 个 Flyway 迁移脚本（V1-V29），路径：`backend/DataOcean/src/main/resources/db/migration/`
...
> 共 6 个公共组件，路径：`frontend/src/components/`
...
└── chart/
    ...
    ├── ChartExporter.vue    — 图表导出（图片/PDF）
    ├── ChartTypeSwitcher.vue — 图表类型切换器（柱/线/饼等）
    └── QueryExplanation.vue — 查询结果自然语言解释展示
```
...
├── module/                    — 业务模块（共 13 个，340 个 Java 文件）
...
│   │   └── client: PythonKnowledge/PythonRag, scheduler: VectorIndexTask, support: ChunkSplitter
...
│   ├── reranker.py               — 规则加权重排（表名命中/可信度/废弃惩罚）
│   ├── chunker.py                — 分块策略（表级 + skills.md 按标题切分）
...
└── prompt/                        — Prompt 管理（spec 014 Python 端）
    ...
    ├── token_budget.py           — Token 预算控制（4000 总量，按优先级裁剪）
