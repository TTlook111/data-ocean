...
| Java user/auth/permission modules | IAM-SIMPLE-1 B0–B4 (including B4-A and batches 1–6) code and automation are in `1f0a5f4`; 23 controllers migrated; current code switch commit is `1e2f458`; B5 local-development switch and core browser acceptance are complete on IT-GO-1225; B6 cleanup started on `codex/iam-s1-b6-cleanup` (`fa0ca89`, pushed) but is **not closed out** — legacy permission controllers, services, mappers, `frontend/src/api/admin/permission.ts`, the Caffeine permission cache and the legacy permission tables remain |
...
B6 cleanup started on `codex/iam-s1-b6-cleanup` (`fa0ca89`, pushed to origin) but is **not closed out**. Removed so far: `LoginUser`/`UserContext` legacy role fields, the JWT `roles`/`permissions` claims, `LoginVO`/`CurrentUserVO` role fields, `UserDetailsServiceImpl` legacy permission loading, and the frontend `guards.ts` / `stores/auth.ts` legacy permission arrays. Still present: `DatasourcePermissionController` / `AccessPolicyController` / `AccessApprovalController`, their services and mappers, `frontend/src/api/admin/permission.ts`, the `PermissionCalculatorImpl` Caffeine permission cache, and the legacy permission tables. The frozen-checklist replacements `authProtocolVersion` / `sessionEpoch` / `iam-s1:session:*` have **zero** references in Java main code, and `jwt:blacklist:{jti}` / `user:token-version:{userId}` remain the only session-invalidation mechanism in use — so B6 rows 497/498 are half-done (old claims deleted, new protocol never built) and `tokenVersion` is deliberately still live. `5f13efd` on the same branch is **not** a deletion commit: it continues the IAM-S1 migration (query readiness and conversation, replacing old permission-access logic with the S1 path). Read this branch as IAM-S1 migration close-out rather than a pure B6 deletion branch; that framing was decided by the project owner on 2026-09-25. Verified on 2026-09-25 for `5f13efd`: Java `mvn test` 581 passed / 0 failures / 0 errors / 0 skipped, frontend Vitest 70 passed (12 files), `npm run build` exit 0. V53 is permanently unused; P9 must use V58 or higher. The B5 handbook remains the generic procedure for other environments. Automation passing does not mean B5 is production-complete. Do not map or backfill old grants, mix permission algorithms, or delete old permissions during initial setup.
