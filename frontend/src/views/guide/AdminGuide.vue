...
const steps: StepDef[] = [
  {
    ...
    color: 'blue',
    title: '接入数据源',
    desc: '在「数据源」中添加 MySQL 连接信息，点击测试确认连通后启用。支持多个数据源独立管理。',
    action: { label: '去添加数据源', to: '/admin/data-sources' },
    detailTitle: '数据源状态说明',
    details: [
  ...
  },
  {
    ...
    color: 'green',
    title: '采集元数据',
    desc: '在「采集任务」中触发元数据采集，系统自动扫描目标库的所有表结构、字段类型、索引和注释。',
    action: { label: '去同步', to: '/admin/collections' },
    detailTitle: '采集方式说明',
    details: [
      { label: '手动触发', desc: '立即执行一次全量采集，适合首次接入或结构变更后', tag: 'info' },
      { label: '定时调度', desc: '在「采集任务」的调度页签中配置 cron 表达式，系统按计划自动采集', tag: 'info' },
    ],
  ...
  },
  {
    ...
    color: 'orange',
    title: '治理元数据',
    desc: '在「治理总览」查看自动校验结果，问题明细在「问题中心」；修复缺失注释、不一致命名等问题。质量分越高，AI 查询越准确。',
    action: { label: '去治理', to: '/admin/governance' },
  ...
  },
  {
    ...
    color: 'purple',
    title: '发布快照',
    desc: '治理完成后在「版本发布」中发布一个冻结版本。发布后的快照作为 AI 查询的可信数据依据。',
    action: { label: '去发布', to: '/admin/releases' },
    detailTitle: '快照版本说明',
    details: [
  ...
  },
  {
    ...
    color: 'sky',
    title: '生成知识库',
    desc: '在「语义知识」中选择快照，AI 自动生成 skills.md 业务文档。人工审核确认后发布，内容将向量化进入 RAG。',
    action: { label: '去生成', to: '/admin/semantics/knowledge' },
