...
1. **导航可见性**：AppShell.vue 使用“一级业务域侧边栏 + 内容区二级工作区导航”。一级业务域通过 `menuGroups` 定义，二级入口通过 `workspaceLinks` 定义；只要用户拥有某业务域下任一二级页面权限，就可以看到该业务域入口。
...
5. **新增后台页面归属**：新增 `/admin/*` 页面前，先参考 `../docs/development/后台信息架构与导航规范.md` 判断属于数据源中心、治理中心、语义中心、权限与开放、运营与安全或系统设置；不要直接把技术页面加入侧边栏。
...
- 现代扁平化管理后台
- 左侧固定侧边栏只展示一级业务域，内容区顶部工作区导航展示二级功能入口（已实现在 AppShell.vue）
- 顶部栏显示当前页面标题 + 用户信息
...
3. [ ] 页面归属是否符合 `docs/development/后台信息架构与导航规范.md`？
4. [ ] 如果需要导航入口，是加到 `workspaceLinks`，而不是侧边栏一级菜单？
5. [ ] 页面内的操作按钮是否需要额外权限判断？
6. [ ] 是否使用了 `post-login-page` class？
7. [ ] 是否遵循了 page-header + content 的结构？
8. [ ] 加载态、空状态、错误态是否都处理了？
9. [ ] 是否使用了 CSS 变量而非硬编码颜色？
10. [ ] 图标是否来自 lucide-vue-next？
