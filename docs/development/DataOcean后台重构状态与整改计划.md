...
2026-09-13 已执行第一次真实环境验收。环境、部分接口、页面、旧 URL 和截图通过，但核心业务链路存在 500、400、403、数据源上下文错误和移动端导航缺失，因此整体不通过。
...
- 顶栏必须提供明确的菜单按钮。
- 点击后打开侧栏抽屉，抽屉内完整展示一级业务域和二级工作区。
- 选择二级工作区后关闭抽屉并完成导航。
- 不允许继续使用 `display: none` 隐藏侧栏而不提供替代入口。
...
**修改要求**：按第 4 节完成桌面展开、折叠和移动端导航；删除内容区顶部的二级工作区条，不改变现有 URL 与路由元数据语义。

### A-09 修复移动端后台导航

当前小屏幕样式直接隐藏侧栏，截图中只剩二级工作区且无法切换业务域。应新增顶栏菜单按钮和侧栏抽屉，并完成键盘、焦点锁定、Escape 关闭和遮罩点击验证。
...
- 移动端表格不得以不可发现的方式裁切关键列，需提供响应式卡片或明确横向滚动。
...
8. 新侧栏两级导航的桌面、折叠、移动端、直接 URL 和键盘操作；
9. 34 个旧 URL 回归。
...
- 浏览器机器证据：`output/playwright/轨道A浏览器验收证据.json`
