...
轨道 A 和导航结构冻结后再开始。目标输入为 `docs/development/guides/DataOcean-完整权限体系设计.md` 的简明版 1.3 / IAM-SIMPLE-1。已补齐上次中断的设计，并按用户“上手简单、每项权限作用清楚”的要求补全全部功能说明、模板优先的角色配置、常规授权三项必选和按需展开限制；保留数据授权、合并、字段保护、审批、实际权限预览、开发契约和 B0～B6 替换验收设计。当前仍为设计稿，业务端点消费清单与具体契约待 B0 核实，不代表代码、数据库或真实验收已完成。

用户要求权限简单易懂，采用“角色管功能、部门默认数据与个人/角色授权管可查数据、后台角色绑定负责源”的模型。管理员首屏使用中文名称、作用与例子；查询/SQL/导出功能只在角色配置，数据授权不重复开关，不建设多层委派或数字优先级。

替换严格按“建立并验收新权限 → 切换且真实复验通过 → 删除旧权限”。不做旧授权映射、回填、双读双写或兼容兜底；切换前保留旧体系运行，新体系独立配置验收。账号、密码、真实组织、业务资产、会话与审计保留；新角色关系/数据授权重新初始化。B6 清理独立于 B1 建表/初始化升级，不让普通启动提前删除旧权限；真实数据库切换/清理留待具体实施。
