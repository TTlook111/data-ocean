...
轨道 A 和导航结构冻结后再开始。目标输入为 `docs/development/guides/DataOcean-完整权限体系设计.md` 的简明版 1.4 / IAM-SIMPLE-1。已补齐上次中断的设计，并按用户“上手简单、每项权限作用清楚”的要求补全全部功能说明、模板优先的角色配置、常规授权三项必选和按需展开限制；保留数据授权、合并、字段保护、审批、实际权限预览、开发契约和 B0～B6 替换验收设计；新增 IAM-SIMPLE-1 唯一事实源、同名权限码不继承旧关系、首个新系统管理员显式初始化、B0 入库交付物与 B6 删除/保留清单约束。当前仍为设计稿，业务端点消费清单与具体契约待 B0 核实，不代表代码、数据库或真实验收已完成。
