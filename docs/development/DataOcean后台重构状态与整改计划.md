...
| 浏览器自动化 | 45 页面、38 断言、129 通过、3 项不可构造；未解释 console/page/request 错误均为 0 | 否 |
...
3 个不可构造项为 F7/F8/F9，原因和代码级边界已记录在最终证据及下文验收约束中。
...
本次复验按真实流程执行并通过：采集快照 10、质量检查和问题处理、快照审核发布、知识文档 27/28/29 生成/审核/索引/发布、Prompt 停用/启用、真实 NL2SQL 查询和会话推荐问题恢复。浏览器以 1440×1000 桌面视口从登录开始执行；最终证据记录 45 个页面、38 个断言，其中 129 个通过，F7/F8/F9 因当前系统没有可达输入而记录为不可构造。console error、page error、request failure 均无未解释项。
