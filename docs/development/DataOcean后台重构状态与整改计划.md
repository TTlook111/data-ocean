...
轨道 A 和导航结构冻结后再开始。目标输入为 `docs/development/guides/DataOcean-完整权限体系设计.md` 的 IAM-1 新设计稿，当前只完成设计文档重写，待设计评审与业务端点消费清单确认，不代表 B1～B6 已实现。

用户明确采用新体系直接替换：废弃既有权限配置/算法，不做权限映射、回填或兼容运行。以用户、多角色、主部门为主体，分开功能、业务数据、MANAGE 治理和 GRANT 授权，前后端成对改造工作区/Tab/操作/API。账号、组织、业务资产等基础数据保留；真实权限清理/新初始化在后续维护窗口确认后执行。
