...
1. 数据库备份只完成文件完整性校验，尚未在独立 MySQL 实例完成恢复验证。
2. 当前只有一个真实用户，未覆盖“启用但没有 S1 绑定的用户”和“只有旧权限、没有 S1 权限的用户”两个负向场景。
3. 尚未初始化普通问数用户、业务分析人员等业务角色、负责数据源、表/字段授权或审批人。
4. “访问审批（旧）”仍保留，本轮不处理。
5. 旧权限体系已在 B6 中移除：`RoleController`、`PermissionController`、`DatasourcePermissionController`、`AccessPolicyController`、`AccessApprovalController`、旧问数链路（`QueryController`、`PythonAgentClientImpl`、`PermissionCalculator`、`DatasourceAccessService`）、旧角色权限实体与 Mapper、前端旧页面与 `api/admin/permission.ts`、Python 旧 Agent 包全部删除；6 张旧表由 `V58` 删除。`permission_change_log` 与 `access_approval_request` 按冻结清单保留为只读历史。账号、部门、数据源、元数据、知识、会话、审计等业务数据未受影响。
6. 本轮没有生产环境验收；不能把 B5 写成无任何遗留项的完全完成。
7. **仍有一处遗留**（冻结清单第 497/498 行）：替代物 `authProtocolVersion` / `sessionEpoch` / `iam-s1:session:*` 在 Java 主代码中仍为 **0 处引用**，`jwt:blacklist:{jti}` / `user:token-version:{userId}` 仍是唯一在用的会话失效机制，`tokenVersion` 仍在使用。**替代会话机制建立并验收前不要删除它**。
8. 同一分支上的 `5f13efd` **不是删除提交**：它延续 IAM-S1 迁移（数据源就绪检查与查询对话，把旧的权限访问逻辑替换为 S1 路径）。因此该分支应理解为 **IAM-S1 迁移收尾分支**，而不是纯粹的 B6 删除分支；此定性由项目负责人在 2026-09-25 决定，不拆分该提交。
9. 2026-09-25 对 `5f13efd` 的验证结果：Java `mvn test` **581** 通过、0 失败、0 错误、0 跳过；前端 Vitest **12 文件 70** 通过；`npm run build` exit code 0。
...
| Java 单元测试 | B4 基线 **557**；令牌与密钥加固后 **603**；**B6 后为 585 通过、0 失败、0 错误、0 跳过**（603 − 18 个随旧类删除的用例，无保留用例丢失） | 否 |
| 前端 Vitest | B4 基线 **11 文件 67**；**B6 后为 12 文件 73 通过**（新增导航可解析性断言 3 个） | 否 |
| 前端生产构建 | `vue-tsc` 与 Vite 通过 | 否 |
| Python 单元测试 | B4 记录 204；本机实测基线 **207**；令牌加固后 **228**；**B6 后为 105 通过**（228 − 123 个旧 Agent/sandbox/SSE 用例，逐文件核对吻合） | 否 |
| Flyway | `IT-GO-1225` 本机开发 MySQL 已按 V51→V52→V54→V55→V56→V57→**V58** 迁移至 V58（V58 = B6 删除 6 张旧权限表），Flyway 失败记录为 0；`iam_s1_*` 仍为 14 张；表数 56→70→64。V53 永久不使用。该事实不推导其他机器或生产环境 | 否 |
...
| 轨道 B-B5 本机切换和核心验收 | `IT-GO-1225` 本机开发环境已迁移至 V57；bootstrap 已完成，固定功能目录 54 项，userId=1 绑定受保护的 `IAM_S1_SYSTEM_ADMIN`，`iam_s1_data_grant=0`；核心服务和浏览器验收通过，旧组织正式入口已移除。仍缺独立 MySQL 恢复演练和双用户负向场景。B6 已执行完成（批次 0～4）：旧权限代码全部删除，6 张旧表由 `V58` 删除 | 否 |
| 内部服务令牌加固（2026-09-25） | `/internal/*` 此前由 8 处重复的公开默认值 `dataocean-internal-default` 保护，其中 `InternalMetadataController` 与 Python `infra/auth.py` **完全没有启动守卫**，且 `application.yml` 把 `spring.profiles.active` 硬编码为 `dev`，使两个已有的守卫形同虚设（它们读的正是这个 property）。现已：取消全部默认值（缺失/含空白/短于 32 字符即拒绝启动，且不依赖 profile）；Java 用单个 `InternalTokenFilter` + `SecurityConfig` 要求 `ROLE_INTERNAL`（fail-closed，共用同一 `RequestMatcher`）取代三处口径不一的方法内校验；两侧改用常量时间比较并先编码 UTF-8；删除无调用方且无认证的 Python `/internal/health`；Python 收敛到 `settings.internal_token` 单一来源。本地取值来自两个被 Git 忽略的文件。**已完成端到端实测（2026-09-25，本机）**：无令牌/错误令牌/正确令牌 → Java `/internal/ai-config` 得 **403 / 403 / 200**；`/api/admin/users` 无登录得 401（内部过滤器未越权拦截）；Python `/internal/sql/pools/dashboard` 得 **403 / 403 / 200**；Python `/internal/health` 得 **404**（已删除）、`/health` 得 **200**；Python 用自身令牌回调 Java `/internal/ai-config` 成功（`/internal/config/reload` 返回 `{"status":"ok"}`），证明两侧令牌实践一致；无令牌时 Java 以 `IllegalStateException` 中止启动（`InternalTokenValidator:53`，`BUILD FAILURE` exit 1），Python 以 `ValidationError` 中止（exit 1） | 否 |
| 登录令牌签名密钥加固（2026-09-25） | `jwt.secret` 的公开默认值 `dataocean-local-dev-secret-key-change-me-at-least-256-bits` 在 `application.yml` 与 `application-dev.yml` 各有一份副本；**dev 才是默认激活的 profile，只改基础文件不生效**。持有该值即可伪造任意用户的登录态，且更换它会让所有已签发令牌失效。现已：两处默认值全部取消（`${JWT_SECRET:}`）；校验收敛到唯一读取方 `JwtTokenProvider#buildSecretKey`（缺失/纯空白/含空白/不足 32 字节即拒绝启动，报错含属性名、本地文件路径与生成命令）；`config/application-local.example.yml` 的可用占位符改为空值 + 说明；本地真实值来自被忽略的 `config/application-local.yml`。已实测确认 `spring.config.import` 引入的本地文件**优先于** `application-dev.yml` 自身的空占位符（在 dev profile 下解析 `jwt.secret` 得 44 字符、`dataocean.internal.token` 得 64 字符）。**已完成实测（2026-09-25，本机）**：注释掉 `jwt.secret` 后 Java 以 `IllegalStateException`（`JwtTokenProvider.buildSecretKey:160`）在构造期中止启动，`BUILD FAILURE` exit 1；恢复正常值后 `Started DataOceanApplication in 7.835 seconds`，`/internal/ai-config` 无令牌 403、正确令牌 200 | 否 |
...
| 权限与组织 | 授权管理 `/admin/access`；授权配置 `/admin/access/iam`；访问申请与审批 `/admin/access/iam-approvals`；组织、角色与负责源 `/admin/access/iam-organization`；访问审批（旧）`/admin/access/approvals`（范围外保留） |
