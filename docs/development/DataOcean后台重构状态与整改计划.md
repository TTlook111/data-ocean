...
| Java 单元测试 | B4 最新基线 **557** 通过；B5 准备轮复跑为 **563** 通过、0 失败、0 错误；2026-09-25 对 `5f13efd` 复跑为 **581**；令牌与密钥加固后为 **603** 通过、0 失败、0 错误、0 跳过（新增 `InternalTokenValidatorTest` 10 个、`InternalTokenFilterTest` 6 个、`JwtTokenProviderTest` 6 个） | 否 |
| 前端 Vitest | B4 最新基线 **11 文件 67** 通过；2026-09-25 对 `5f13efd` 复跑为 **12 文件 70** 通过 | 否 |
| 前端生产构建 | `vue-tsc` 与 Vite 通过 | 否 |
| Python 单元测试 | 204 通过、4 跳过、1 警告（B4 记录）；2026-09-25 本机改动前实测为 **207** 通过、4 跳过；内部令牌加固后为 **228** 通过、4 跳过、1 警告（新增 `tests/test_internal_auth.py` 21 个） | 否 |
