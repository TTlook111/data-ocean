...
| 提升为模板 | `POST /api/admin/audit-logs/{id}/promote-template` | 在审计详情中展示 |
...
- AI 配置、供应商、模型测试、模型同步、向量维度检测和重新向量化。
- 告警规则列表、创建、修改和启停。
...
查询审计支持数据源筛选、统计、详情和提升为模板。性能分析当前只支持全局慢查询分页，因此进入该 Tab 时隐藏数据源上下文，并明确筛选能力边界。
...
AI 配置保留供应商、模型、连接测试、模型同步、Embedding 维度检测和重新向量化。重新向量化必须展示影响范围、当前状态和失败信息，不作为普通保存操作的一部分。
...
**仍存在的缺口**（上表其余各行不变，且新增两项）：

| 缺口 | 说明 |
| --- | --- |
| 术语责任人筛选、批量处理明细相关 | 问题中心的责任人筛选仍需后端 `assigneeId` 参数（见 `docs/development/后续开发.md` P13） |
| 审批列表未按申请人收窄 | 属权限模型本身，与后续权限体系重构一并处理（§3.12） |
| 旧规避方案未回收 | 资产目录与术语关联字段仍在客户端过滤；`resolveReadinessActionPath` 的 `focus` 参数已无消费方（已改为直接落数据源详情） |
