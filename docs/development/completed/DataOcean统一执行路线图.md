...
**与当前整改文档的关系**：本文档只记录已完成的历史重构路线；当前状态、轨道 A 整改和后续新增功能统一见 `docs/development/DataOcean后台重构状态与整改计划.md`。
...
- 当前整改文档中的新功能任务不直接从本历史阶段分支继续做；应在轨道 A 完成后，从当时最新 `develop` 或 `main` 按任务性质重新创建分支
...
后台侧栏按业务链路组织，不按代码包名机械分组。此处“一级业务域侧栏 + 内容区二级工作区导航”是历史实现；已批准的新目标是一级、二级都放入侧栏，后续新增页面以 `docs/development/DataOcean后台重构状态与整改计划.md` 为准。

- 血缘属于数据资产治理链路，应靠近元数据、质量治理、字段治理，而不是孤立放在审计运维里
- 权限入口按组织结构优先：部门 → 岗位/角色 → 用户
- 当前没有实现、且属于后续开发的页面，不提前写进导航
- 后续开发项不进入本文档侧栏方案
- 当前阶段只有在确实涉及前端导航时，才调整 `AppShell.vue` 的 `menuGroups` / `workspaceLinks`
