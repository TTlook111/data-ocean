...
**与当前整改文档的关系**：本文档只记录已完成的历史重构路线；当前状态、轨道 A 整改和后续新增功能统一见 `DataOcean后台重构状态与整改计划.md`。
...
后台侧栏按业务链路组织，不按代码包名机械分组。此处“一级业务域侧栏 + 内容区二级工作区导航”是历史实现；已批准的新目标是一级、二级都放入侧栏，后续新增页面以 `DataOcean后台重构状态与整改计划.md` 为准。
