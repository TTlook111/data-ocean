...
B6 删掉了旧问数链路，`/api/iam-s1/query` 成为唯一路径。此前该路径**从未真正跑过**：
Java 侧测试 mock 了 Python 客户端，Python 侧测试直接构造 schema，两边从未对接。
本次用真实登录 → 建授权 → 提问走完整链路，结果如下。

**权限层逐项通过**：

| 验证项 | 结果 |
| --- | --- |
| 登录响应字段 | `expiresIn/passwordChanged/realName/token/tokenType/userId/username`，**无 roles/permissions** |
| 能力摘要 | `systemAdmin=true`、54 个全局功能码、3 个数据源能力 |
| 无数据授权时 | 可查数据源为 `[]` —— 系统管理员不会因身份自动获得业务数据权限 |
| 系统管理员给自己授权 | 被拒（"不能修改本人的后台负责源"）——职责分离 |
| 整库 ALLOW | 被拒（"数据源级授权只能用于字段级、记录级或 DENY"）——不存在一刀切的库级放行 |
| 给他人建字段级授权 | 成功 |
| 数据授权 ≠ 查询能力 | 仅有授权、无角色时 `queryUse=false`；需同时具备角色能力与数据授权 |
| 按角色模板建角色并分配 | 成功后 `queryUse=true`、可查数据源出现、ask 返回 200 |

**发现并修复两个阻断缺陷**（提交 `aae530a`）：

1. **日期序列化与契约不符**：`buildSnapshot` 把 `LocalDateTime` 原样放进快照，而 Python 声明
   为 `str`；`PythonRestClientConfig` 用静态 `RestClient.builder()`，其 Jackson converter 不套用
   本应用的日期配置，把日期写成时间戳数组 → Python 422，**任何查询都无法开始**。
   应用自身接口的日期是正常 ISO 字符串，所以只看接口表现发现不了。
2. **`COUNT(*)` 被无条件拒绝**：`_expand_stars` 只展开投影位置的星号，兜底守卫却用
   `tree.find(exp.Star)`，会命中 `COUNT(*)` 里作为函数参数的星号。
   "表里有多少条记录"的正确答案就是 `COUNT(*)`，必然被拒。

同时把 `IamS1PythonClientImpl` 的 4xx/5xx 报错从"HTTP 状态异常"改为带上状态码与响应体——
缺陷 1 正是靠它才定位到。

**后续又发现同一类问题的三个环节，已一并修复**（提交 `6936d07`）：

S1 查询链路**从未支持"不引用任何列"的查询**（`COUNT(*)` 或常量输出）。三个环节是：

3. **来源追踪把"合法地没有列来源"当成"来源缺失"**：`COUNT(*)` 不引用列，`sources` 为空是
   正确的；Java 的来源完整性检查把空来源一律判为缺失，整个结果被拒
   （"结果来源不完整"）。修复：Python 对此类输出显式写 `sourceKind=NO_COLUMN_SOURCE`，
   Java 只对带该标记的条目允许空来源；未声明却为空仍拒绝，保持 fail-closed。
4. **输出列名与数据库实际列名不一致**：数据库用表达式原文当未加别名投影的列名
   （`COUNT(*)`），而 sqlglot 的 `alias_or_name` 返回 `*`。修复：校验器给"无别名且不是裸列"
   的投影分配确定性别名（`s1_c1`…），列名由本服务决定，不再依赖数据库命名规则。
5. **空 `usedColumns` 被当成权限变化**：`isSubsetOfCurrent` 与 `hasCompletePersistedEvidence`
   都要求 `usedColumns` 非空，而 `COUNT(*)` 必然为空，于是完成阶段被判
   `REJECTED_ON_RECHECK`，用户看到"权限已变化，请重新查询"——与实际原因完全无关。
   修复：新增"无列证据"判定（trace 中每条输出都带 `NO_COLUMN_SOURCE` 且来源为空）；
   完成阶段、读取阶段、持久化证据三处一致放宽，否则任务能完成却读不回来。

顺带修掉一个**误导性文案**：`complete()` 的 `catch(BusinessException)` 覆盖整个完成阶段
（最终保护、落库、会话消息与摘要刷新），其中任何业务异常都被报成"权限已变化"。
对用户保留该文案，但把真实原因记入日志——本次排查正是先被它误导过一轮。

**当前状态：S1 问数链路端到端已打通。** 实测（真实登录 + 真实授权 + 真实提问）：

```text
status=COMPLETED  columns=[{name: s1_c1}]  data=[{s1_c1: 12}]  maskedFields={}
```

即 `demo_order` 的 12 条订单，经由 RAG 检索 → SQL 生成 → AST 校验（含别名分配）→
沙箱执行 → 来源追踪 → Java 最终保护 → 落库 → 读回再审全链路返回。

**一句话结论**：旧权限体系确实删干净了；问数链路在此前**从未真正跑过**，
经本次验证发现并修复 5 个阻断缺陷后端到端打通。

**遗留**：结果列名对用户显示为 `s1_c1` 这类技术名（模型没给聚合起别名），
S1 的 SQL 生成提示词应要求对表达式投影使用业务化别名；浏览器端（前端页面）验证仍未做。
