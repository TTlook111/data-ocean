...
**仍未通过的一项**：SQL 生成与校验通过后，执行阶段报"数据源连接失败"。
Java 侧 `/api/admin/datasources/1/test-connection` **返回成功**（MySQL 8.0.46），
说明凭据可解密、库可达；Python 侧沙箱建池日志正常，但执行异常未落入可见日志。
已缩小到"执行阶段的 connect 类异常"，**根因未定位**。

**一句话结论**：旧权限体系确实删干净了；但"问数功能可用"这一条，在修掉上述两个缺陷后
已推进到执行阶段，**尚未打通**。
